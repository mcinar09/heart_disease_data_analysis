...
author: "Mukadder Cinar, CUNY, Graduate Center"
format: html
standalone: true
...
The analysis sought to provide insightful categorizations of patients based on their heart disease data, potentially paving the way for personalized medicine and targeted healthcare interventions. The project underscored the efficacy of machine learning in healthcare analytics and emphasized the significance of systematic data exploration in deriving meaningful conclusions from complex medical datasets.

## Methodology

In this study, a systematic approach to analyze cardiovascular disease (CVD) data was applied, leveraging a blend of machine learning techniques to unravel the complexities associated with heart disease diagnosis and patient management. The methodology was structured to not only identify inherent patterns within the dataset but also to enhance the precision and applicability of the findings in clinical settings.

Initially, the K-Nearest Neighbors (KNN) classification algorithm was utilized, a method renowned for its efficacy in pattern recognition within medical datasets. This was followed by an in-depth analysis using K-means clustering, conducted in two distinct rounds to assess the stability and reliability of the identified clusters. The first round of K-means clustering provided a baseline for patient groupings, while the second round, involving an adjustment in the “nstart” parameter, served to test the consistency of these groupings.

Further enriching the analysis, Principal Component Analysis (PCA) was implemented to visualize the clusters in a reduced-dimensional space, thereby offering a clearer understanding of the data’s structure. Lastly, hierarchical clustering, executed via both Ward and Complete linkage methods, was applied to explore and validate the data’s inherent groupings at different levels of granularity.

The comprehensive nature of this methodology, encompassing both classification and clustering techniques, was designed to yield insights that are robust, nuanced, and directly applicable to the improvement of cardiovascular health outcomes.
...
The plot revealed that the majority of patients experience asymptomatic chest pain, indicating no chest pain, while the fewest exhibit symptoms of typical chest pain.\
...
The plot highlighted that the largest proportion of heart disease patients are male. Additionally, it indicated that the counts of up-slope and flat-slope are quite similar, while the count of down-slope is relatively low.
...
The plot demonstrated a noticeable increase in the number of patients after the age of 40. Cholesterol levels exhibit some outliers on the higher side. Furthermore, the distribution of resting blood pressure is skewed to the right, whereas the distribution of Maximum heart rate is skewed to the left.
...
The plot highlighted a notably higher rate of heart disease in patients experiencing asymptomatic chest pain. Regarding the ST Slope in relation to the presence of Heart Disease, it's evident that when the ST slope is flat, the likelihood of the disease is substantially higher (no disease: disease = 16%: 30%), whereas approximately 10% of patients with the up-slope exhibit the disease.
...
The plot depicting Sex versus Heart Disease revealed that the likelihood of the disease is greater in males (approximately 30% without the disease and 37% with the disease), whereas in females, it is considerably lower (approximately 24% without the disease and 8% with the disease).

Regarding fasting blood sugar, a level below 120 mg/dl is considered normal. The graph illustrated that when fasting blood sugar levels are elevated, the proportion of individuals with and without the disease is roughly equal.
...
Prior to the application of the K-NN classification algorithm, the dataset underwent a thorough inspection for missing values.
...
Subsequently, the data was normalized using the “scale()” function, a standard preprocessing step facilitated by the ‘caret’ package in R. This scaling process is crucial to ensure that the variables contribute equally to the analysis, avoiding any bias due to the scale of measurements
...
**Accuracy**: The choice of **`k = 13`** was based on achieving the highest accuracy. Accuracy is a measure of how often the model correctly predicts the target variable. A high accuracy with this k-value suggested that the model is generally effective in classifying the data correctly.
...
\
K-means clustering is a popular unsupervised machine learning algorithm used for partitioning a dataset into a set of distinct, non-overlapping subgroups or clusters. The K-means clustering algorithm in the heart disease dataset was employed to uncover hidden groupings or clusters among the data points.The algorithm aims to partition the data such that the variance within each cluster is minimized [@k-means].
...
The scree plot indicated that two clusters would be optimal for the dataset in question. However, there was also substantial evidence to justify the exploration of three clusters, presenting an alternative perspective on the potential segmentation of the data.

To firmly establish the correct k-value, an alternative method called the Average Silhouette method was employed. This technique aids in identifying the optimal number of clusters (k) in unsupervised clustering, like K-Means. It assesses the degree of similarity between data points within their assigned clusters versus other clusters. A higher Average Silhouette score signifies more distinct and well-defined clusters, and the k-value corresponding to the highest average score is selected as the optimal number of clusters [@kassambara].
...
After conducting both the Elbow method and the Average Silhouette method, the optimal k-value was determined as 2.
...
For the heart disease dataset, the significance of the clusters was evaluated to ensure they were not mere random groupings. Consistency in patient grouping was examined by running the k-means algorithm from different starting points. If identical patients consistently clustered differently, this could indicate that the clusters were not meaningfully related to patient characteristics. Consequently, the algorithm was executed twice to test the robustness of the clustering outcome.
...
K-means clustering was performed twice on the heart disease data by changing the `nstart` value in the second round which refers to the number of random initialization of centers. The K-means algorithm is sensitive to the initial placement of the centroids [@k-means]. To mitigate this, `nstart` specifies how many times the algorithm should be run with different random initialization. The best output in terms of within-cluster sum of squares is kept.
...
In the contingency table obtained from two rounds of clustering, the diagonal cells represented the count of data points that remained in the same cluster for both rounds. For instance, the count at the intersection of Cluster 1 in Round 1 and Cluster 1 in Round 2 indicated the number of patients who stayed in Cluster 1 throughout.

High counts in the diagonal cells, relative to the off-diagonal cells, were indicative of high level of consistency, signifying that considerable number of patients consistently grouped into the same cluster in both clustering rounds.

In contrast, higher counts in the off-diagonal cells compared to the diagonal cells suggested that numerous patients moved to different clusters in the subsequent round, implying variability or instability in the clustering outcomes.

In the contingency table presented, the diagonal values (51, 125) indicated lower counts compared to the off-diagonal ones. While 125 instances remained in Cluster 2 during the second round, the high count of off-diagonal (displaced) values (60, 67) implied that K-means clustering might not have been effective or robust for this dataset, as evidenced by the lack of consistent clustering results.
...
It's observed that in cluster 1, the averages for age, resting blood pressure, cholesterol levels, resting electrocardiographic readings, and oldpeak are higher, whereas the average maximum heart rate is lower.
...
Within Cluster 1, the analysis revealed that mean values for age, resting blood pressure, cholesterol levels, resting electrocardiographic readings, and oldpeak were elevated. Conversely, the average maximum heart rate was observed to be lower in comparison both rounds of clustering.
...
#| fig-cap: Distribution of Exercise-induced angina and Fasting Blood Sugar on the Round-2 Clusters.
#| fig-cap-location: top
#| code-fold: true
...
The clusters derived from the K-means algorithm were visualized using Principal Component Analysis (PCA) in order to uncover patterns and groupings that might not be apparent in the high-dimensional space.
...
**Understanding Data Structure**: The visualization in the reduced PCA space allowed for the observation of patient groupings as determined by the k-means algorithm. Clusters that appeared distinct and well-separated in the PCA plot indicated the successful identification of meaningful patterns in the data.
...
To comprehend the principal components more thoroughly, PCA Loadings were analyzed. PCA loadings indicate the contribution of each original variable to the principal components, shedding light on the variables that most significantly differentiate the clusters[@principa].
...
The PCA loadings revealed that PC1 was predominantly influenced by cholesterol levels, suggesting that this component may be reflective of patient health in relation to metabolic factors. In contrast, PC2 influenced significantly by maximum heart rate (MaxHR) and age, albeit in opposite directions, potentially representing a combination of cardiovascular efficiency (reflected by heart rate) and age-related factors.

Through the PCA loadings and subsequent cluster visualization, insights into the underlying characteristics of patient groups were obtained. PC1 was primarily indicative of cholesterol-related variations, while PC2 captures a blend of heart rate and age factors. This dimensional reduction, coupled with k-means clustering, provided a detailed understanding of patient health profiles, which could assist in devising targeted healthcare strategies. However, these findings should be considered exploratory and validated further with clinical expertise to ensure their relevance and accuracy in medical research or practice.

In summary, the analysis of the heart disease dataset using k-means clustering with two clusters, demonstrated moderate consistency in patient groupings across two rounds of clustering. While a significant portion of patients were grouped into the same cluster in both rounds, indicating potentially meaningful groupings, there was also notable movement between clusters. This suggests that while there may be some stable patterns in the data, there is also a degree of variability in how patients are grouped. These findings highlight the importance of considering multiple rounds of clustering and possibly exploring additional clustering methods or incorporating more domain-specific knowledge for a comprehensive understanding of patient categorizations.
...
3.  Average Linkage: In average linkage clustering, the distance between two clusters is defined as the average distance between each data point in the first cluster and every data point in the second cluster. Application: It is useful for creating clusters where each member is similar on average to the members of its cluster. Characteristics: This method provides a balance between the sensitivity of the complete method to outliers and the tendency of the Ward's method to create clusters of similar sizes. However, it can sometimes create chaining effects where clusters end up being elongated and straggly [@msc2023].
    ...
    In this analysis, the hierarchical clustering was performed using the two most common linkage methods (the ward and the complete linkage), subsequently compared to each other.
...
Based on the dendrogram, the optimal cut height for dividing the dendrogram into two clusters falls within the range of 25-30. This outcome aligned with the result obtained using the Silhouette method for K-means clustering, reaffirming the choice of two clusters as the most suitable number.

After partitioning the dendrogram with K= 2, the table that provides the count of observations within each cluster was generated.
...
Subsequent to the hierarchical clustering process, cluster labels were reassigned to the original dataset to facilitate analysis of cluster composition. The dataset was summarized by cluster to gain insights into the mean or standard deviation values of each variable within each cluster.
